...
#' #' @param palette_continuous The viridis-family color palette to use for continuous variables. Defaults to "E", which corresponds to the cividis color palette, which has fewer distinct hues, resulting in fewer false segmentations of the underlying data driven by hue transitions. Must be a length-1 character string from "A" to "H" corresponding to the same codes used by `scale_color_viridis_d()` and similar functions to refer to the eight color families available in the viridis package.
