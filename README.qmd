...
However, there's a *lot* more to know–if you're interested, keep reading!
...
Thus far, we focused on graphs that use color (hue, luminance, and saturation) to communicate differences in the underlying data. `ggplot.plus` is designed to revamp how `ggplot2` approaches color to yield graphs that are more accessible and interpretable.

However, *any* graph that uses color to communicate difference can only be so accessible and interpretable. A small but sizable number of humans are completely colorblind, and color perception weakens as we age. Additionally, many people still read and/or view scientific publications in grayscale, wherein colors may not be as readily distinguished (unless they vary in luminance).

As such, data viz experts would remind us that there are other visual channels (ways of communicating difference) out there! One of these, shape, is a useful channel for communicating difference in scatterplots like those we've made so far. However, `ggplot2` only makes available the same 26 shapes available in base R:

```{r Rs base shapes }
pch_values = 0:25

# Create a simple plot displaying the shapes
plot(pch_values,
     rep(1, length(pch_values)), 
     pch = 0:25, cex = 2)
```

While there is some variation between the shapes available, many of the shapes are perceptually very similar to one another, making them difficult to distinguish quickly \[research has suggested that humans more readily distinguish shapes when they vary along three axes: 1) Openness, 2) Spikiness, and 3) Intersectionality\].

In addition, only the last five of these shapes have separate outline (stroke) and interior (fill) portions, allowing them to bear separate outline and fill colors, and these five shapes are very similar with respect to the three axes outlined above. This limits their usefulness in instances where communicating difference through color and shape is desired (or, for that matter, when color is being useful for emphasis or visual effect).

As such, we designed a custom version of `geom_point()`, `geom_point_plus()`, that introduces the ability to access nine new shapes that A) intentionally vary along the three axes described above as much as possible and B) can bear separate stroke and fill colors:
...
These more distinctive shapes can be used to communicate difference whenever using color to do so is undesirable:
...
ggplot(iris[sample(1:nrow(iris), 30, replace = FALSE), ], #<--RESTRICT DATA VOLUME TO PREVENT OVERPLOTTIN FOR ILLUSTRATION.
...
As the example above shows, you can specify a new shape legend title right inside `geom_point_plus()` using the `legend_title` parameter. You can also use the `shape_values` parameter to specify which shapes you want to use:
...
As you might expect, you can also access these features using `geom_plus(geom = "point_plus")` instead:
...
As previously noted, all these shapes have separate fill and color aesthetics, so they can be styled and even mapped to variables (although we wouldn't necessarily recommend it):
...
You can use any of the new shapes as your points in "regular" scatterplots (i.e., those that don't map shape to data) too, although, to do this (at present), you must:
...
Of course, users who are already familiar with these graph-design issues and concepts may find these warnings distracting or repetitive. They can all be disabled by setting `silence_warnings` inside `geom_plus()` to `TRUE`.
