...
format: 
  html: 
    ...
    title: "ggplot_plus Readme and Quick-Start Guide"
    echo: true
    code-overflow: wrap
  gfm:
...
To begin using the `ggplot_plus` package, you’ll first need to install it from GitHub using the devtools package:
...
`ggplot_plus` also introduces a new palette of shapes designed to be more distinctive than the base point shapes available in R, accessible using `geom_point_plus()`:
...
The above graphs demonstrates how `ggplot_plus`'s tools rethink the default design features of `ggplot2`. The intention is to yield a more opinionated, more universal product more quickly so you can spend less time fiddling with your graphs than you otherwise might.
