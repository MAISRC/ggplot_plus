...
1.  **Better Defaults:**\
    Many default settings in `ggplot2` are functional but sub-optimal from design and accessibility standpoints. `ggplot.plus` attempts to improve upon these—adjusting colors, shapes, gridlines, spacing, typefaces, etc. to yield cleaner, more accessible visuals from the outset.

2.  **Override-ability:**\
    All defaults set by `ggplot.plus`'s tools can be overridden by the user, either through standard `ggplot2` syntax or using `_plus` variants with arguments. You’re rarely (if ever) forced to keep anything these tools generate.

3.  **Modularity:**\
    Each tool in `ggplot.plus` is designed to be additive. You can use just one of them (e.g., `theme_plus()` or `geom_plus()`) or combine them as needed.
...
The `ggplot.plus` package includes a convenience function, `palettes_plus()`, that activates a carefully chosen default palette for use whenever the `color` or `fill` aesthetics are required by your subsequent plots:
...
However, you can easily adjust this behavior using the `begin_*` and `end_*` parameters, which range from 0 to 1:

-   `begin_discrete = 0` would include the dark purple portion of the color range for discrete scales.

-   `end_discrete = 1` would include the light yellow portion.

```{r bringing in dark purple}
palettes_plus(begin_discrete = 0) #<--ALLOW DARK PURPLE RANGE.
...
To see what the color palette looks like for continuous variables, let's map color to a different numeric variable in the data set:

```{r check out cividis}
ggplot(iris,
       mapping = aes(x = Petal.Length, 
           y = Sepal.Length)) +
  geom_point(mapping = aes(color = Petal.Width), show.legend = FALSE) #<--MAP COLOR TO A CONTINUOUS VARIABLE INSTEAD, SUPPRESS THE LEGEND FOR NOW.
```
...
```{r turbo}
palettes_plus(begin_discrete = 0, end_discrete = 1, palette = "H") #<--SWITCH TO TURBO PALETTE FOR ITS FULL RANGE. 
...
Note: **Turbo** has two dark regions (at both ends), which may be indistinguishable to users with total or partial colorblindness (\< 1% of the population). To mitigate this, you can trim one half of the palette using `begin_*` or `end_*` values around 0.5 to include only the "red" or "blue" side.

### A Few Usage Tips

`palettes_plus()` is called automatically when the package is loaded. It sets the default color and fill palettes globally for all `ggplot2` plots thereafter. It needn't be called again unless you want to change something. For example, let's change the palette for discrete variables to a modified *viridis* with the darkest shades removed:

```{r reset to virids}
palettes_plus(begin_discrete = 0.28, end_discrete = 1, palette = "D") #<--RESET TO VIRIDIS, DON'T INCLUDE THE DARK REGION.
```
...
```{r restore purple}
palettes_plus(begin_discrete = 0, end_discrete = 1) #<--RESTORE PURPLE END OF COLOR RANGE AND RETAIN YELLOW RANGE (WHICH, BY DEFAULT, WOULD BE CUT OFF).
...
In the latter case, overplotting will still be an issue without one more tweak: We can make the points semi-transparent so that points that stack on top of one another "bleed through" each other and result in a darker point (colors will "blend" as well, if applicable):
...
2.  **Local mapping**: Inside `aes()` within a specific `geom_*()` function. These only apply to that one layer.

3.  **Constant values**: Set inside or outside `aes()` within a geom or inside of `aes()` within `ggplot()`. These are fixed and override any mapping at or above their same level.
...
-   The **alpha aesthetic** is set to a constant value (0.3), again **only locally** for the jittered points.
...
If you’d like to see additional geoms supported, you can request them using the Issues feature on the package's Github page.
...
There are a few theme considerations that apply only to color bars, but, to see them, we need to map a continuous variable to color/fill to get such a legend:

```{r cont color bar theme }
ggplot(iris,
       mapping = aes(x = Petal.Length, 
           y = Sepal.Length)) +
  geom_plus(geom = "point", 
            mapping = aes(fill = Petal.Width), #<--CHANGE TO CONTINUOUS VARIABLE.
            alpha = 0.3) +
  scale_x_continuous_plus(name = "Petal length (cm)", 
                          labels = c(1, "", 3, "", 5, "", 7)) +
  scale_y_continuous_plus(name = "Sepal length (cm)") + 
...
The changes here include longer and fatter tick marks, a black border, and larger dimensions for easier reading.

Note that colorbars can have the same issue as axes, where the ends don't necessarily get labels at or near them, depending upon how breaks are decided. `ggplot.plus` includes functions equivalent to `scale_x/y_continuous_plus()` for `color` and `fill` as well:
...
ggplot(iris,
  ...
  scale_y_continuous_plus(name = "Sepal length (cm)") + 
  theme_plus() +
  scale_fill_continuous_plus(name = "Petal width (cm)") #<--GET BETTER AXIS BREAKS
```
