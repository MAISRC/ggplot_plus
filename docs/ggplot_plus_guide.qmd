...
-   `begin_discrete = 0.28` would disable the dark purple portion of the color range.
...
Sometimes, this will be necessary to get both elements to fit. However, if there is enough room for them to sit side by side, you can set `move_top_legend_down` to `TRUE` inside `yaxis_title_plus()` to move the legend down into the same row as the relocated y axis title:

```{r relocating the legend}
#| fig-height: 6
#| fig-width: 9
